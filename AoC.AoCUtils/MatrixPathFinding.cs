...
{
    private Dictionary<MatrixPoint<T>, int> Cost{ get; set; } = [];
    private SortedList<int, MatrixPoint<T>> Candidates { get; set; } = new SortedList<int, MatrixPoint<T>>(new DuplicateKeyComparer());
    private Dictionary<MatrixPoint<T>, MatrixPoint<T>> Previous { get; set; } = [];
    private HashSet<MatrixPoint<T>> Visited { get; set; } = [];
